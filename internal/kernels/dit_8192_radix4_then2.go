...
// forwardDIT8192Radix4Then2Complex64 computes a 8192-point forward FFT using
...
func forwardDIT8192Radix4Then2Complex64(dst, src, twiddle, scratch []complex64) bool {
	const n = 8192
	...
	var stage2Backing [n]complex64
	stage2 := stage2Backing[:]
	...
	var stage3Backing [n]complex64
	stage3 := stage3Backing[:]
	...
	var stage4Backing [n]complex64
	stage4 := stage4Backing[:]
	...
	var stage5Backing [n]complex64
	stage5 := stage5Backing[:]
	...
	var stage6Backing [n]complex64
	stage6 := stage6Backing[:]
	...
	for base := 0; base < n; base += 4096 {
		for j := range 1024 {
			...
			a0 := stage5[idx0]
			a1 := mathpkg.MulComplex64(w1, stage5[idx1])
			a2 := mathpkg.MulComplex64(w2, stage5[idx2])
			a3 := mathpkg.MulComplex64(w3, stage5[idx3])
	...
	for j := range 4096 {
		tw := tw[j]
		a := stage6[j]
		b := mathpkg.MulComplex64(tw, stage6[j+4096])
		work[j] = a + b
		work[j+4096] = a - b
	}
...
// inverseDIT8192Radix4Then2Complex64 computes a 8192-point inverse FFT using
// radix-4-then-2 Decimation-in-Time (DIT) algorithm for complex64 data.
//
// Uses conjugated twiddle factors and applies 1/N scaling.
func inverseDIT8192Radix4Then2Complex64(dst, src, twiddle, scratch []complex64) bool {
	const n = 8192
