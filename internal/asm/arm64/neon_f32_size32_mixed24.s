...
// ·neonInv32 has been relocated to neon_f32_size32_radix2.s (its only
// remaining consumer, per AGENTS.md "Retiring a kernel" step 1 — this file
// is being retired in favor of the shared radix4_loop core). This file
// keeps its own private copy so its inverse path is unaffected until the
// rest of the retirement checklist removes it.
DATA neon32m24Inv<>+0(SB)/4, $0x3d000000 // 1/32
GLOBL neon32m24Inv<>(SB), RODATA, $4
...
neon32m24_bitrev_loop:
	CMP  $32, R0
	BGE  neon32m24_stage1
	...
	ADD  $1, R0, R0
	B    neon32m24_bitrev_loop

neon32m24_stage1:
	// Stage 1: 8 radix-4 butterflies (no twiddles)
	MOVD $0, R14

neon32m24_stage1_loop:
	CMP  $32, R14
	BGE  neon32m24_stage2
	...
	ADD  $4, R14, R14
	B    neon32m24_stage1_loop

neon32m24_stage2:
	// Stage 2: radix-4 with twiddles, size=16, step=2
	MOVD $0, R14

neon32m24_stage2_base:
	CMP  $32, R14
	BGE  neon32m24_stage3

	MOVD $0, R15

neon32m24_stage2_j:
	CMP  $4, R15
	BGE  neon32m24_stage2_next
	...
	ADD  $1, R15, R15
	B    neon32m24_stage2_j

neon32m24_stage2_next:
	ADD  $16, R14, R14
	B    neon32m24_stage2_base

neon32m24_stage3:
	// Stage 3: radix-2, size=32, step=1
	MOVD $0, R0

neon32m24_stage3_loop:
	CMP  $16, R0
	BGE  neon32m24_done
	...
	ADD  $1, R0, R0
	B    neon32m24_stage3_loop

neon32m24_done:
	CMP  R8, R20
	BEQ  neon32m24_return_true

	MOVD $0, R0
neon32m24_copy_loop:
	CMP  $32, R0
	BGE  neon32m24_return_true
	LSL  $3, R0, R1
	ADD  R8, R1, R2
	MOVD (R2), R2
	ADD  R20, R1, R3
...
TEXT ·InverseNEONSize32MixedRadix24Complex64Asm(SB), NOSPLIT, $0-97
	MOVD dst+0(FP), R8
	MOVD src+24(FP), R9
	MOVD twiddle+48(FP), R10
	MOVD scratch+72(FP), R11
	...
	CMP  $32, R13
	BNE  neon32m24_inv_return_false
	...
	MOVD R8, R20
	CMP  R8, R9
	BNE  neon32m24_inv_use_dst
	MOVD R11, R8

neon32m24_inv_use_dst:
	// Bit-reversal permutation (mixed-radix 2/4)
	MOVD $0, R0

neon32m24_inv_bitrev_loop:
	CMP  $32, R0
	BGE  neon32m24_inv_stage1
	...
	ADD  $1, R0, R0
	B    neon32m24_inv_bitrev_loop

neon32m24_inv_stage1:
	// Stage 1: 8 radix-4 butterflies (no twiddles)
	MOVD $0, R14

neon32m24_inv_stage1_loop:
	CMP  $32, R14
	BGE  neon32m24_inv_stage2
	...
	ADD  $4, R14, R14
	B    neon32m24_inv_stage1_loop

neon32m24_inv_stage2:
	// Stage 2: radix-4 with conjugated twiddles, size=16, step=2
	MOVD $0, R14

neon32m24_inv_stage2_base:
	CMP  $32, R14
	BGE  neon32m24_inv_stage3

	MOVD $0, R15

neon32m24_inv_stage2_j:
	CMP  $4, R15
	BGE  neon32m24_inv_stage2_next
	...
	ADD  $1, R15, R15
	B    neon32m24_inv_stage2_j

neon32m24_inv_stage2_next:
	ADD  $16, R14, R14
	B    neon32m24_inv_stage2_base

neon32m24_inv_stage3:
	// Stage 3: radix-2, size=32, step=1 (conjugated twiddles)
	MOVD $0, R0

neon32m24_inv_stage3_loop:
	CMP  $16, R0
	BGE  neon32m24_inv_scale
	...
	ADD  $1, R0, R0
	B    neon32m24_inv_stage3_loop

neon32m24_inv_scale:
	CMP  R8, R20
	BEQ  neon32m24_inv_scale_apply

	MOVD $0, R0
neon32m24_inv_copy_loop:
	CMP  $32, R0
	BGE  neon32m24_inv_scale_apply
	LSL  $3, R0, R1
	ADD  R8, R1, R2
	MOVD (R2), R2
	ADD  R20, R1, R3
	MOVD R2, (R3)
	ADD  $1, R0, R0
	B    neon32m24_inv_copy_loop

neon32m24_inv_scale_apply:
	MOVD $neon32m24Inv<>(SB), R1
	FMOVS (R1), F0
	MOVD $0, R0

neon32m24_inv_scale_loop:
	CMP  $32, R0
	BGE  neon32m24_inv_return_true
	LSL  $3, R0, R1
	ADD  R20, R1, R1
	FMOVS 0(R1), F2
	FMOVS 4(R1), F3
