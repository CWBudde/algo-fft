...
// ===========================================================================
// NEON Size-16 Radix-2 FFT Kernels for ARM64
// ===========================================================================
...
neon16r2_base_loop:
	CMP  R13, R17
	BGE  neon16r2_next_size

	MOVD $0, R0                // j

neon16r2_inner_loop:
	CMP  R15, R0
	BGE  neon16r2_next_base
	...
	ADD  $1, R0, R0
	B    neon16r2_inner_loop

neon16r2_next_base:
	ADD  R14, R17, R17
	B    neon16r2_base_loop

neon16r2_next_size:
	LSL  $1, R14, R14
	B    neon16r2_size_loop

neon16r2_done:
	CMP  R8, R20
	BEQ  neon16r2_return_true
...
TEXT ·InverseNEONSize16Radix2Complex64Asm(SB), NOSPLIT, $0-97
	...
	MOVD R8, R20
	CMP  R8, R9
	BNE  neon16r2_inv_use_dst
	MOVD R11, R8

neon16r2_inv_use_dst:
	// Bit-reversal permutation
	MOVD $0, R0

neon16r2_inv_bitrev_loop:
	CMP  $16, R0
	BGE  neon16r2_inv_stage
	...
	ADD  $1, R0, R0
	B    neon16r2_inv_bitrev_loop

neon16r2_inv_stage:
	MOVD $2, R14               // size

neon16r2_inv_size_loop:
	CMP  $16, R14
	BGT  neon16r2_inv_done
...
neon16r2_inv_base_loop:
	CMP  R13, R17
	BGE  neon16r2_inv_next_size

	MOVD $0, R0                // j

neon16r2_inv_inner_loop:
	CMP  R15, R0
	BGE  neon16r2_inv_next_base
	...
	ADD  $1, R0, R0
	B    neon16r2_inv_inner_loop

neon16r2_inv_next_base:
	ADD  R14, R17, R17
	B    neon16r2_inv_base_loop

neon16r2_inv_next_size:
	LSL  $1, R14, R14
	B    neon16r2_inv_size_loop

neon16r2_inv_done:
	CMP  R8, R20
	BEQ  neon16r2_inv_scale

	MOVD $0, R0
neon16r2_inv_copy_loop:
	CMP  $16, R0
...
neon16r2_inv_scale_loop:
	CMP  $16, R0
	BGE  neon16r2_inv_return_true
	LSL  $3, R0, R1
	ADD  R20, R1, R1
	FMOVS 0(R1), F2
	FMOVS 4(R1), F3
