//go:build amd64 && !purego

// ===========================================================================
// SSE3 Size-256 Radix-2 FFT Kernels for AMD64
// ===========================================================================

#include "textflag.h"

// Forward transform, size 256, complex64, radix-2
TEXT ·ForwardSSE3Size256Radix2Complex64Asm(SB), NOSPLIT, $0-97
	// Load parameters
	MOVQ dst+0(FP), R8
	MOVQ src+24(FP), R9
	MOVQ twiddle+48(FP), R10
	MOVQ scratch+72(FP), R11
	LEAQ ·bitrev256_r2(SB), R12
	MOVQ src_len+32(FP), R13

	CMPQ R13, $256
	JNE  size256_r2_return_false

	// Validate all slice lengths >= 256
	MOVQ dst_len+8(FP), AX
	CMPQ AX, $256
	JL   size256_r2_return_false

	MOVQ twiddle_len+56(FP), AX
	CMPQ AX, $256
	JL   size256_r2_return_false

	MOVQ scratch_len+80(FP), AX
	CMPQ AX, $256
	JL   size256_r2_return_false

	// Select working buffer
	CMPQ R8, R9
	JNE  size256_r2_use_dst
	MOVQ R11, R8

size256_r2_use_dst:
	// Bit-reversal permutation (radix-2)
	XORQ CX, CX

size256_r2_bitrev_loop:
	MOVQ (R12)(CX*8), DX
	MOVQ (R9)(DX*8), AX
	MOVQ AX, (R8)(CX*8)
	INCQ CX
	CMPQ CX, $256
	JL   size256_r2_bitrev_loop

	// Stage 1: size=2, half=1, step=128
	MOVQ R8, SI
	MOVQ $128, CX
size256_r2_stage1_loop:
	MOVSD (SI), X0
	MOVSD 8(SI), X1
	MOVAPS X0, X2
	ADDPS X1, X0
	SUBPS X1, X2
	MOVSD X0, (SI)
	MOVSD X2, 8(SI)
	ADDQ $16, SI
	DECQ CX
	JNZ  size256_r2_stage1_loop

	// Stage 2: size=4, half=2, step=64
	MOVQ R8, SI
	MOVQ $64, CX
size256_r2_stage2_loop:
	MOVQ $2, DX
size256_r2_stage2_inner:
	MOVSD (SI), X0
	MOVSD 16(SI), X1
	MOVQ $2, AX
	SUBQ DX, AX
	SHLQ $6, AX
	MOVSD (R10)(AX*8), X10
	MOVAPS X10, X11
	SHUFPS $0x00, X11, X11
	MOVAPS X10, X12
	SHUFPS $0x55, X12, X12
	MOVAPS X1, X13
	SHUFPS $0xB1, X13, X13
	MULPS X11, X1
	MULPS X12, X13
	ADDSUBPS X13, X1
	MOVAPS X0, X2
	ADDPS X1, X0
	SUBPS X1, X2
	MOVSD X0, (SI)
	MOVSD X2, 16(SI)
	ADDQ $8, SI
	DECQ DX
	JNZ size256_r2_stage2_inner
	ADDQ $16, SI
	DECQ CX
	JNZ size256_r2_stage2_loop

	// Stage 3: size=8, half=4, step=32
	MOVQ R8, SI
	MOVQ $32, CX
size256_r2_stage3_loop:
	MOVQ $4, DX
size256_r2_stage3_inner:
	MOVSD (SI), X0
	MOVSD 32(SI), X1
	MOVQ $4, AX
	SUBQ DX, AX
	SHLQ $5, AX
	MOVSD (R10)(AX*8), X10
	MOVAPS X10, X11
	SHUFPS $0x00, X11, X11
	MOVAPS X10, X12
	SHUFPS $0x55, X12, X12
	MOVAPS X1, X13
	SHUFPS $0xB1, X13, X13
	MULPS X11, X1
	MULPS X12, X13
	ADDSUBPS X13, X1
	MOVAPS X0, X2
	ADDPS X1, X0
	SUBPS X1, X2
	MOVSD X0, (SI)
	MOVSD X2, 32(SI)
	ADDQ $8, SI
	DECQ DX
	JNZ size256_r2_stage3_inner
	ADDQ $32, SI
	DECQ CX
	JNZ size256_r2_stage3_loop

	// Stage 4: size=16, half=8, step=16
	MOVQ R8, SI
	MOVQ $16, CX
size256_r2_stage4_loop:
	MOVQ $8, DX
size256_r2_stage4_inner:
	MOVSD (SI), X0
	MOVSD 64(SI), X1
	MOVQ $8, AX
	SUBQ DX, AX
	SHLQ $4, AX
	MOVSD (R10)(AX*8), X10
	MOVAPS X10, X11
	SHUFPS $0x00, X11, X11
	MOVAPS X10, X12
	SHUFPS $0x55, X12, X12
	MOVAPS X1, X13
	SHUFPS $0xB1, X13, X13
	MULPS X11, X1
	MULPS X12, X13
	ADDSUBPS X13, X1
	MOVAPS X0, X2
	ADDPS X1, X0
	SUBPS X1, X2
	MOVSD X0, (SI)
	MOVSD X2, 64(SI)
	ADDQ $8, SI
	DECQ DX
	JNZ size256_r2_stage4_inner
	ADDQ $64, SI
	DECQ CX
	JNZ size256_r2_stage4_loop

	// Stage 5: size=32, half=16, step=8
	MOVQ R8, SI
	MOVQ $8, CX
size256_r2_stage5_loop:
	MOVQ $16, DX
size256_r2_stage5_inner:
	MOVSD (SI), X0
	MOVSD 128(SI), X1
	MOVQ $16, AX
	SUBQ DX, AX
	SHLQ $3, AX
	MOVSD (R10)(AX*8), X10
	MOVAPS X10, X11
	SHUFPS $0x00, X11, X11
	MOVAPS X10, X12
	SHUFPS $0x55, X12, X12
	MOVAPS X1, X13
	SHUFPS $0xB1, X13, X13
	MULPS X11, X1
	MULPS X12, X13
	ADDSUBPS X13, X1
	MOVAPS X0, X2
	ADDPS X1, X0
	SUBPS X1, X2
	MOVSD X0, (SI)
	MOVSD X2, 128(SI)
	ADDQ $8, SI
	DECQ DX
	JNZ size256_r2_stage5_inner
	ADDQ $128, SI
	DECQ CX
	JNZ size256_r2_stage5_loop

	// Stage 6: size=64, half=32, step=4
	MOVQ R8, SI
	MOVQ $4, CX
size256_r2_stage6_loop:
	MOVQ $32, DX
size256_r2_stage6_inner:
	MOVSD (SI), X0
	MOVSD 256(SI), X1
	MOVQ $32, AX
	SUBQ DX, AX
	SHLQ $2, AX
	MOVSD (R10)(AX*8), X10
	MOVAPS X10, X11
	SHUFPS $0x00, X11, X11
	MOVAPS X10, X12
	SHUFPS $0x55, X12, X12
	MOVAPS X1, X13
	SHUFPS $0xB1, X13, X13
	MULPS X11, X1
	MULPS X12, X13
	ADDSUBPS X13, X1
	MOVAPS X0, X2
	ADDPS X1, X0
	SUBPS X1, X2
	MOVSD X0, (SI)
	MOVSD X2, 256(SI)
	ADDQ $8, SI
	DECQ DX
	JNZ size256_r2_stage6_inner
	ADDQ $256, SI
	DECQ CX
	JNZ size256_r2_stage6_loop

	// Stage 7: size=128, half=64, step=2
	MOVQ R8, SI
	MOVQ $2, CX
size256_r2_stage7_loop:
	MOVQ $64, DX
size256_r2_stage7_inner:
	MOVSD (SI), X0
	MOVSD 512(SI), X1
	MOVQ $64, AX
	SUBQ DX, AX
	SHLQ $1, AX
	MOVSD (R10)(AX*8), X10
	MOVAPS X10, X11
	SHUFPS $0x00, X11, X11
	MOVAPS X10, X12
	SHUFPS $0x55, X12, X12
	MOVAPS X1, X13
	SHUFPS $0xB1, X13, X13
	MULPS X11, X1
	MULPS X12, X13
	ADDSUBPS X13, X1
	MOVAPS X0, X2
	ADDPS X1, X0
	SUBPS X1, X2
	MOVSD X0, (SI)
	MOVSD X2, 512(SI)
	ADDQ $8, SI
	DECQ DX
	JNZ size256_r2_stage7_inner
	ADDQ $512, SI
	DECQ CX
	JNZ size256_r2_stage7_loop

	// Stage 8: size=256, half=128, step=1
	MOVQ R8, SI
	MOVQ $128, DX
size256_r2_stage8_inner:
	MOVSD (SI), X0
	MOVSD 1024(SI), X1
	MOVQ $128, AX
	SUBQ DX, AX
	MOVSD (R10)(AX*8), X10
	MOVAPS X10, X11
	SHUFPS $0x00, X11, X11
	MOVAPS X10, X12
	SHUFPS $0x55, X12, X12
	MOVAPS X1, X13
	SHUFPS $0xB1, X13, X13
	MULPS X11, X1
	MULPS X12, X13
	ADDSUBPS X13, X1
	MOVAPS X0, X2
	ADDPS X1, X0
	SUBPS X1, X2
	MOVSD X0, (SI)
	MOVSD X2, 1024(SI)
	ADDQ $8, SI
	DECQ DX
	JNZ size256_r2_stage8_inner

	// Copy to dst if needed
	MOVQ dst+0(FP), R9
	CMPQ R8, R9
	JE   size256_r2_done

	MOVQ $128, CX
	MOVQ R8, SI
size256_r2_copy_loop:
	MOVUPS (SI), X0
	MOVUPS X0, (R9)
	ADDQ $16, SI
	ADDQ $16, R9
	DECQ CX
	JNZ size256_r2_copy_loop

size256_r2_done:
	MOVB $1, ret+96(FP)
	RET

size256_r2_return_false:
	MOVB $0, ret+96(FP)
	RET

// Inverse transform, size 256, complex64, radix-2
TEXT ·InverseSSE3Size256Radix2Complex64Asm(SB), NOSPLIT, $0-97
	// Load parameters
	MOVQ dst+0(FP), R8
	MOVQ src+24(FP), R9
	MOVQ twiddle+48(FP), R10
	MOVQ scratch+72(FP), R11
	LEAQ ·bitrev256_r2(SB), R12
	MOVQ src_len+32(FP), R13

	CMPQ R13, $256
	JNE  size256_r2_inv_return_false

	// Validate all slice lengths >= 256
	MOVQ dst_len+8(FP), AX
	CMPQ AX, $256
	JL   size256_r2_inv_return_false

	MOVQ twiddle_len+56(FP), AX
	CMPQ AX, $256
	JL   size256_r2_inv_return_false

	MOVQ scratch_len+80(FP), AX
	CMPQ AX, $256
	JL   size256_r2_inv_return_false

	// Select working buffer
	CMPQ R8, R9
	JNE  size256_r2_inv_use_dst
	MOVQ R11, R8

size256_r2_inv_use_dst:
	// Bit-reversal permutation (radix-2)
	XORQ CX, CX

size256_r2_inv_bitrev_loop:
	MOVQ (R12)(CX*8), DX
	MOVQ (R9)(DX*8), AX
	MOVQ AX, (R8)(CX*8)
	INCQ CX
	CMPQ CX, $256
	JL   size256_r2_inv_bitrev_loop

	MOVUPS ·maskNegHiPS(SB), X14

	// Stage 1: size=2, half=1, step=128
	MOVQ R8, SI
	MOVQ $128, CX
size256_r2_inv_stage1_loop:
	MOVSD (SI), X0
	MOVSD 8(SI), X1
	MOVAPS X0, X2
	ADDPS X1, X0
	SUBPS X1, X2
	MOVSD X0, (SI)
	MOVSD X2, 8(SI)
	ADDQ $16, SI
	DECQ CX
	JNZ  size256_r2_inv_stage1_loop

	// Stage 2: size=4, half=2, step=64
	MOVQ R8, SI
	MOVQ $64, CX
size256_r2_inv_stage2_loop:
	MOVQ $2, DX
size256_r2_inv_stage2_inner:
	MOVSD (SI), X0
	MOVSD 16(SI), X1
	MOVQ $2, AX
	SUBQ DX, AX
	SHLQ $6, AX
	MOVSD (R10)(AX*8), X10
	XORPS X14, X10
	MOVAPS X10, X11
	SHUFPS $0x00, X11, X11
	MOVAPS X10, X12
	SHUFPS $0x55, X12, X12
	MOVAPS X1, X13
	SHUFPS $0xB1, X13, X13
	MULPS X11, X1
	MULPS X12, X13
	ADDSUBPS X13, X1
	MOVAPS X0, X2
	ADDPS X1, X0
	SUBPS X1, X2
	MOVSD X0, (SI)
	MOVSD X2, 16(SI)
	ADDQ $8, SI
	DECQ DX
	JNZ size256_r2_inv_stage2_inner
	ADDQ $16, SI
	DECQ CX
	JNZ size256_r2_inv_stage2_loop

	// Stage 3: size=8, half=4, step=32
	MOVQ R8, SI
	MOVQ $32, CX
size256_r2_inv_stage3_loop:
	MOVQ $4, DX
size256_r2_inv_stage3_inner:
	MOVSD (SI), X0
	MOVSD 32(SI), X1
	MOVQ $4, AX
	SUBQ DX, AX
	SHLQ $5, AX
	MOVSD (R10)(AX*8), X10
	XORPS X14, X10
	MOVAPS X10, X11
	SHUFPS $0x00, X11, X11
	MOVAPS X10, X12
	SHUFPS $0x55, X12, X12
	MOVAPS X1, X13
	SHUFPS $0xB1, X13, X13
	MULPS X11, X1
	MULPS X12, X13
	ADDSUBPS X13, X1
	MOVAPS X0, X2
	ADDPS X1, X0
	SUBPS X1, X2
	MOVSD X0, (SI)
	MOVSD X2, 32(SI)
	ADDQ $8, SI
	DECQ DX
	JNZ size256_r2_inv_stage3_inner
	ADDQ $32, SI
	DECQ CX
	JNZ size256_r2_inv_stage3_loop

	// Stage 4: size=16, half=8, step=16
	MOVQ R8, SI
	MOVQ $16, CX
size256_r2_inv_stage4_loop:
	MOVQ $8, DX
size256_r2_inv_stage4_inner:
	MOVSD (SI), X0
	MOVSD 64(SI), X1
	MOVQ $8, AX
	SUBQ DX, AX
	SHLQ $4, AX
	MOVSD (R10)(AX*8), X10
	XORPS X14, X10
	MOVAPS X10, X11
	SHUFPS $0x00, X11, X11
	MOVAPS X10, X12
	SHUFPS $0x55, X12, X12
	MOVAPS X1, X13
	SHUFPS $0xB1, X13, X13
	MULPS X11, X1
	MULPS X12, X13
	ADDSUBPS X13, X1
	MOVAPS X0, X2
	ADDPS X1, X0
	SUBPS X1, X2
	MOVSD X0, (SI)
	MOVSD X2, 64(SI)
	ADDQ $8, SI
	DECQ DX
	JNZ size256_r2_inv_stage4_inner
	ADDQ $64, SI
	DECQ CX
	JNZ size256_r2_inv_stage4_loop

	// Stage 5: size=32, half=16, step=8
	MOVQ R8, SI
	MOVQ $8, CX
size256_r2_inv_stage5_loop:
	MOVQ $16, DX
size256_r2_inv_stage5_inner:
	MOVSD (SI), X0
	MOVSD 128(SI), X1
	MOVQ $16, AX
	SUBQ DX, AX
	SHLQ $3, AX
	MOVSD (R10)(AX*8), X10
	XORPS X14, X10
	MOVAPS X10, X11
	SHUFPS $0x00, X11, X11
	MOVAPS X10, X12
	SHUFPS $0x55, X12, X12
	MOVAPS X1, X13
	SHUFPS $0xB1, X13, X13
	MULPS X11, X1
	MULPS X12, X13
	ADDSUBPS X13, X1
	MOVAPS X0, X2
	ADDPS X1, X0
	SUBPS X1, X2
	MOVSD X0, (SI)
	MOVSD X2, 128(SI)
	ADDQ $8, SI
	DECQ DX
	JNZ size256_r2_inv_stage5_inner
	ADDQ $128, SI
	DECQ CX
	JNZ size256_r2_inv_stage5_loop

	// Stage 6: size=64, half=32, step=4
	MOVQ R8, SI
	MOVQ $4, CX
size256_r2_inv_stage6_loop:
	MOVQ $32, DX
size256_r2_inv_stage6_inner:
	MOVSD (SI), X0
	MOVSD 256(SI), X1
	MOVQ $32, AX
	SUBQ DX, AX
	SHLQ $2, AX
	MOVSD (R10)(AX*8), X10
	XORPS X14, X10
	MOVAPS X10, X11
	SHUFPS $0x00, X11, X11
	MOVAPS X10, X12
	SHUFPS $0x55, X12, X12
	MOVAPS X1, X13
	SHUFPS $0xB1, X13, X13
	MULPS X11, X1
	MULPS X12, X13
	ADDSUBPS X13, X1
	MOVAPS X0, X2
	ADDPS X1, X0
	SUBPS X1, X2
	MOVSD X0, (SI)
	MOVSD X2, 256(SI)
	ADDQ $8, SI
	DECQ DX
	JNZ size256_r2_inv_stage6_inner
	ADDQ $256, SI
	DECQ CX
	JNZ size256_r2_inv_stage6_loop

	// Stage 7: size=128, half=64, step=2
	MOVQ R8, SI
	MOVQ $2, CX
size256_r2_inv_stage7_loop:
	MOVQ $64, DX
size256_r2_inv_stage7_inner:
	MOVSD (SI), X0
	MOVSD 512(SI), X1
	MOVQ $64, AX
	SUBQ DX, AX
	SHLQ $1, AX
	MOVSD (R10)(AX*8), X10
	XORPS X14, X10
	MOVAPS X10, X11
	SHUFPS $0x00, X11, X11
	MOVAPS X10, X12
	SHUFPS $0x55, X12, X12
	MOVAPS X1, X13
	SHUFPS $0xB1, X13, X13
	MULPS X11, X1
	MULPS X12, X13
	ADDSUBPS X13, X1
	MOVAPS X0, X2
	ADDPS X1, X0
	SUBPS X1, X2
	MOVSD X0, (SI)
	MOVSD X2, 512(SI)
	ADDQ $8, SI
	DECQ DX
	JNZ size256_r2_inv_stage7_inner
	ADDQ $512, SI
	DECQ CX
	JNZ size256_r2_inv_stage7_loop

	// Stage 8: size=256, half=128, step=1
	MOVQ R8, SI
	MOVQ $128, DX
size256_r2_inv_stage8_inner:
	MOVSD (SI), X0
	MOVSD 1024(SI), X1
	MOVQ $128, AX
	SUBQ DX, AX
	MOVSD (R10)(AX*8), X10
	XORPS X14, X10
	MOVAPS X10, X11
	SHUFPS $0x00, X11, X11
	MOVAPS X10, X12
	SHUFPS $0x55, X12, X12
	MOVAPS X1, X13
	SHUFPS $0xB1, X13, X13
	MULPS X11, X1
	MULPS X12, X13
	ADDSUBPS X13, X1
	MOVAPS X0, X2
	ADDPS X1, X0
	SUBPS X1, X2
	MOVSD X0, (SI)
	MOVSD X2, 1024(SI)
	ADDQ $8, SI
	DECQ DX
	JNZ size256_r2_inv_stage8_inner

	// Scale by 1/256
	MOVSS ·twoFiftySixth32(SB), X15
	SHUFPS $0x00, X15, X15
	MOVQ $128, CX
	MOVQ R8, SI
size256_r2_inv_scale_loop:
	MOVUPS (SI), X0
	MULPS X15, X0
	MOVUPS X0, (SI)
	ADDQ $16, SI
	DECQ CX
	JNZ size256_r2_inv_scale_loop

	// Copy to dst if needed
	MOVQ dst+0(FP), R9
	CMPQ R8, R9
	JE   size256_r2_inv_done

	MOVQ $128, CX
	MOVQ R8, SI
size256_r2_inv_copy_loop:
	MOVUPS (SI), X0
	MOVUPS X0, (R9)
	ADDQ $16, SI
	ADDQ $16, R9
	DECQ CX
	JNZ size256_r2_inv_copy_loop

size256_r2_inv_done:
	MOVB $1, ret+96(FP)
	RET

size256_r2_inv_return_false:
	MOVB $0, ret+96(FP)
	RET
