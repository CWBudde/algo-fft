...
// six-step FFT algorithm which decomposes 16384 = 128×128.
//
// Memory layout:
//   - Matrix is 128×128 complex64 values
//   - Each complex64 = 8 bytes (4 bytes real + 4 bytes imag)
//   - Row stride = 128 * 8 = 1024 bytes
//   - Total size = 16384 * 8 = 131072 bytes
//
...
TEXT ·Transpose128x128Complex64AVX2Asm(SB), NOSPLIT, $0-49
    ...
    CMPQ R9, $16384
    JL   transpose128_return_false
    CMPQ R11, $16384
    JL   transpose128_return_false

    // Process 4×4 blocks of complex64: 32 block rows × 32 block cols = 1024 blocks
    // Block (bi, bj) at src: src + (bi*4*128 + bj*4) * 8 = src + bi*4096 + bj*32
    // After transpose:       dst + (bj*4*128 + bi*4) * 8 = dst + bj*4096 + bi*32

    XORQ CX, CX              // CX = bi (block row index, 0..31)

transpose128_bi_loop:
    CMPQ CX, $32
    JGE  transpose128_done

    XORQ DX, DX              // DX = bj (block col index, 0..31)

transpose128_bj_loop:
    CMPQ DX, $32
    JGE  transpose128_bi_next

    // Calculate source block address: src + bi*4096 + bj*32
    MOVQ CX, AX
    SHLQ $12, AX             // bi * 4096
    MOVQ DX, BX
    SHLQ $5, BX              // bj * 32
    ...
    // Calculate dest block address: dst + bj*4096 + bi*32
    MOVQ DX, AX
    SHLQ $12, AX             // bj * 4096
    MOVQ CX, BX
    SHLQ $5, BX              // bi * 32
    ...
    // =========================================================
    // Row stride = 1024 bytes (128 complex64 * 8 bytes each)
    //
    ...
    VMOVUPS 1024(SI), Y1           // row1 (stride = 1024)
    VMOVUPS 2048(SI), Y2           // row2
    VMOVUPS 3072(SI), Y3           // row3
    ...
    VMOVUPS Y10, 1024(DI)          // transposed col1 -> dst row1
    VMOVUPS Y9, 2048(DI)           // transposed col2 -> dst row2
    VMOVUPS Y11, 3072(DI)          // transposed col3 -> dst row3

    // Next block column
    INCQ DX
    JMP  transpose128_bj_loop

transpose128_bi_next:
    INCQ CX
    JMP  transpose128_bi_loop
...
// ===========================================================================
//
// TransposeTwiddle128x128Complex64AVX2Asm performs:
...
//
// Parameters:
//   dst: destination buffer (128×128 complex64)
//   src: source buffer (128×128 complex64)
//   twiddle: twiddle factors (16384 complex64)
//
...
TEXT ·TransposeTwiddle128x128Complex64AVX2Asm(SB), NOSPLIT, $0-73
    ...
    JL   tt128_return_false  // jump if not
    CMPQ R11, $16384         // src length >= 16384?
    JL   tt128_return_false  // jump if not
    CMPQ R13, $16384         // twiddle length >= 16384?
    JL   tt128_return_false  // jump if not

    // Process element by element with twiddle multiplication
    // dst[i*128+j] = src[j*128+i] * twiddle[(i*j) % 16384]
    //
    // Outer loop: i = 0..127 (destination row)
    XORQ CX, CX              // CX = i (clear for loop counter)

tt128_row_loop:
    CMPQ CX, $128            // i < 128?
    JGE  tt128_done          // jump if done

    // R14 = dst row base = dst + i*128*8 = dst + i*1024
    MOVQ CX, AX              // AX = i
    SHLQ $10, AX             // AX = i * 1024 (row stride)
    LEAQ (R8)(AX*1), R14     // R14 = dst + i*1024 (row base address)

    // Inner loop: j = 0..127 (destination column), process 4 at a time
    XORQ DX, DX              // DX = j (clear for loop counter)

tt128_col_loop:
    CMPQ DX, $128            // j < 128?
    JGE  tt128_row_next      // jump to next row if done

    // Load 4 consecutive src elements: src[j*128+i], src[(j+1)*128+i], ...
    // These are NOT contiguous in memory (stride = 1024 bytes)

    // src[j,i] address = src + (j*128 + i)*8 = src + j*1024 + i*8
    MOVQ DX, AX              // AX = j
    SHLQ $10, AX             // AX = j * 1024 (row stride for src)
    MOVQ CX, BX              // BX = i
    SHLQ $3, BX              // BX = i * 8 (element offset within row)
    ADDQ BX, AX              // AX = j*1024 + i*8 (address offset)
    ...
    ADDQ $1024, AX           // AX += 1024 (advance to next row)
    ...
    // Load twiddle factors: twiddle[(i*j) % 16384], twiddle[(i*(j+1)) % 16384], ...
    MOVQ CX, AX              // AX = i (outer loop counter)
    MOVQ DX, BX              // BX = j (inner loop counter)
    IMULQ BX, AX             // AX = i * j (multiply)
    ANDQ $16383, AX          // AX = (i * j) % 16384 (modulo via bitwise AND)
    SHLQ $3, AX              // AX = ((i*j) % 16384) * 8 (offset in bytes)
    ...
    JMP  tt128_col_loop      // continue inner loop

tt128_row_next:
    INCQ CX              // CX += 1 (move to next row)
    JMP  tt128_row_loop  // continue outer loop
...
// ===========================================================================
//
// TransposeTwiddleConj128x128Complex64AVX2Asm performs:
...
TEXT ·TransposeTwiddleConj128x128Complex64AVX2Asm(SB), NOSPLIT, $0-73
    ...
    // Validate lengths
    CMPQ R9, $16384          // dst length >= 16384?
    JL   ttc128_return_false // jump if not
    CMPQ R11, $16384         // src length >= 16384?
    JL   ttc128_return_false // jump if not
    CMPQ R13, $16384         // twiddle length >= 16384?
    JL   ttc128_return_false // jump if not
...
ttc128_row_loop:
    CMPQ CX, $128            // i < 128?
    JGE  ttc128_done         // jump if done

    MOVQ CX, AX              // AX = i
    SHLQ $10, AX             // AX = i * 1024 (row stride)
    LEAQ (R8)(AX*1), R14     // R14 = dst + i*1024 (dst row base address)

    XORQ DX, DX              // DX = j (clear for inner loop counter)

ttc128_col_loop:
    CMPQ DX, $128            // j < 128?
    JGE  ttc128_row_next     // jump to next row if done

    // Load 4 source elements (non-contiguous, stride 1024)
    MOVQ DX, AX              // AX = j
    SHLQ $10, AX             // AX = j * 1024 (row stride for src)
    MOVQ CX, BX              // BX = i
    SHLQ $3, BX              // BX = i * 8 (element offset within row)
    ...
    ADDQ $4, DX              // DX += 4 (process next 4 columns)
    JMP  ttc128_col_loop     // continue inner loop

ttc128_row_next:
    INCQ CX              // CX += 1 (move to next row)
    JMP  ttc128_row_loop // continue outer loop
