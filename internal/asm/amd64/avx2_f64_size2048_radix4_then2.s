...
TEXT ·ForwardAVX2Size2048Radix4Then2Complex128Asm(SB), NOSPLIT, $0-97
	// Load parameters
...
m24_2048_stage2_inner:
	...
	INCQ DX
	JMP  m24_2048_stage2_inner

m24_2048_stage2_next:
	ADDQ $16, CX
	JMP  m24_2048_stage2_outer
...
m24_2048_stage3_inner:
	...
	INCQ DX
	JMP  m24_2048_stage3_inner

m24_2048_stage3_next:
	ADDQ $64, CX
	JMP  m24_2048_stage3_outer
...
m24_2048_stage4_inner:
	...
	INCQ DX
	JMP  m24_2048_stage4_inner

m24_2048_stage4_next:
	ADDQ $256, CX
	JMP  m24_2048_stage4_outer
...
m24_2048_stage5_inner:
	...
	INCQ DX
	JMP  m24_2048_stage5_inner

m24_2048_stage5_next:
	ADDQ $1024, CX
	JMP  m24_2048_stage5_outer
...
m24_2048_stage6_loop:
	...
	INCQ CX
	JMP  m24_2048_stage6_loop

m24_2048_forward_done:
	// Copy results to dst if needed
	MOVQ dst+0(FP), R9
	CMPQ R8, R9
	JE   m24_2048_forward_ret
...
// ===========================================================================
// Inverse transform, size 2048, complex128, radix-4-then-2
// ===========================================================================
TEXT ·InverseAVX2Size2048Radix4Then2Complex128Asm(SB), NOSPLIT, $0-97
	// Load parameters
	MOVQ dst+0(FP), R8       // R8  = dst pointer
	MOVQ src+24(FP), R9      // R9  = src pointer
	MOVQ twiddle+48(FP), R10 // R10 = twiddle pointer
...
m24_2048_inv_stage2_inner:
	...
	INCQ DX
	JMP  m24_2048_inv_stage2_inner

m24_2048_inv_stage2_next:
	ADDQ $16, CX
	JMP  m24_2048_inv_stage2_outer

m24_2048_inv_stage3:
	// ==================================================================
	// Stage 3: 32 groups, each with 16 butterflies (conjugated twiddles)
	// ==================================================================
	XORQ CX, CX
...
m24_2048_inv_stage3_inner:
	...
	INCQ DX
	JMP  m24_2048_inv_stage3_inner

m24_2048_inv_stage3_next:
	ADDQ $64, CX
	JMP  m24_2048_inv_stage3_outer

m24_2048_inv_stage4:
	// ==================================================================
	// Stage 4: 8 groups, each with 64 butterflies (conjugated twiddles)
	// ==================================================================
	XORQ CX, CX
...
m24_2048_inv_stage4_inner:
	...
	INCQ DX
	JMP  m24_2048_inv_stage4_inner

m24_2048_inv_stage4_next:
	ADDQ $256, CX
	JMP  m24_2048_inv_stage4_outer

m24_2048_inv_stage5:
	// ==================================================================
	// Stage 5: 2 groups, each with 256 butterflies (conjugated twiddles)
	// ==================================================================
	XORQ CX, CX
...
m24_2048_inv_stage5_inner:
	...
	INCQ DX
	JMP  m24_2048_inv_stage5_inner

m24_2048_inv_stage5_next:
	ADDQ $1024, CX
	JMP  m24_2048_inv_stage5_outer
...
m24_2048_inv_scale:
	...
	// ==================================================================
	VMOVSD ·twoThousandFortyEighth64(SB), X8
	VBROADCASTSD X8, Y8
