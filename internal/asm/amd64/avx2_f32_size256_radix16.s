...
stage1_twiddle_col_loop:
	...
	// Unrolled inner twiddle work: 16 rows as 8 pairs (2 complex64 at a time)
	// baseIdx walks k = row*col for row = 0..15
	XORQ R14, R14                // R14 = baseIdx

	// pair 0 (rows 0,1) offset 0
	VMOVUPS 0(SI), X0            // 2 complex64
	LEAQ (R14)(CX*1), R13        // baseIdx + col
	VMOVSD (R10)(R14*8), X1      // twiddle0
	VMOVSD (R10)(R13*8), X2      // twiddle1
	VUNPCKLPD X2, X1, X1         // X1=(tw0,tw1)
	VPERMILPS $0xA0, X1, X2      // Re(W)
	VPERMILPS $0xF5, X1, X3      // Im(W)
	VMULPS X2, X0, X4
