...
// ===========================================================================
//
// This file implements FFT transforms using SSE3 instructions for complex64
// (single-precision) data types as a fallback for systems without AVX2 support.
// SSE3 is widely available on modern x86-64 CPUs.
//
// SIMD STRATEGY
// -------------
...
// Imaginary-sign mask for conjugation: [0, -0, 0, -0].
DATA ·sse3NegImagMask<>(SB)/4, $0x00000000
DATA ·sse3NegImagMask<>+4(SB)/4, $0x80000000
DATA ·sse3NegImagMask<>+8(SB)/4, $0x00000000
DATA ·sse3NegImagMask<>+12(SB)/4, $0x80000000
GLOBL ·sse3NegImagMask<>(SB), RODATA|NOPTR, $16

TEXT ·ForwardSSE3Complex64Asm(SB), NOSPLIT, $0-97
	// -----------------------------------------------------------------------
...
sse2_vec_loop:
	...
	// t = w * b (SSE3, 2 complex64)
	MOVAPS X1, X3
	MOVSLDUP X3, X3          // b_real
	MULPS  X2, X3            // b_real * w

	MOVAPS X1, X4
	MOVSHDUP X4, X4          // b_imag
	MOVAPS X2, X5
	SHUFPS $0xB1, X5, X5     // w_shuf
	MULPS  X5, X4            // b_imag * w_shuf

	ADDSUBPS X4, X3          // t = [r0,i0,r1,i1]
...
// ===========================================================================
TEXT ·InverseSSE3Complex64Asm(SB), NOSPLIT, $0-97
	// -----------------------------------------------------------------------
	// PHASE 1: Load parameters and validate inputs
	// -----------------------------------------------------------------------
	MOVQ dst+0(FP), R8       // R8  = dst pointer
...
inv_sse2_vec_loop:
	...
	// t = conj(w) * b (SSE3, 2 complex64)
	MOVUPS ·sse3NegImagMask<>(SB), X6
	MOVAPS X2, X5
	XORPS X6, X5             // w_conj

	MOVAPS X1, X3
	MOVSLDUP X3, X3          // b_real
	MULPS  X5, X3            // b_real * w_conj

	MOVAPS X1, X4
	MOVSHDUP X4, X4          // b_imag
	MOVAPS X5, X7
	SHUFPS $0xB1, X7, X7     // w_conj shuf
	MULPS  X7, X4            // b_imag * w_conj shuf
