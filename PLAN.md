...
- [ ] **Audit the other tests for permutation/twiddle-blind inputs.** The
      impulse-input problem in
      `internal/transform/recursive_integration_test.go` hid a wrong-answer bug
      at every size ≥ 1024 for an entire precision. Those tests are worth
      keeping, but each needs a companion case driven by a broadband signal and
      compared bin-by-bin against `internal/reference`. Ask the same question of
      the Bluestein, Rader and mixed-radix test vectors — the reflex to test
      with an impulse is not specific to that file.
